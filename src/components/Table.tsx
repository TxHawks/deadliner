...
export default function Table({ lastDoc, pretrial }: TableProps) {
  ...
  const pretrialAsDate = pretrial 
    ? new Date(pretrial)
    : undefined;

  const deadlines = [
    { name: 'דיון מקדמי', date: getDeadline({ startDate: lastDocAsDate,  after: 30}), },
    pretrialAsDate && { name: 'הודעה על דיון מקדמי', date: getDeadline({ startDate: pretrialAsDate,  before: 20}), },
    { name: 'קביעת פגישת מהות', date: getDeadline({ startDate: lastDocAsDate,  after: 45}), },
    { name: 'הגשת בקשות', date: getDeadline({ startDate: lastDocAsDate,  after: 53}), },
    ...
    { name: 'שאלונים', date: getDeadline({ startDate: lastDocAsDate,  after: 30}), },
    { name: 'תשובה לשאלונים', date: getDeadline({ startDate: lastDocAsDate,  after: 60}), },
    { name: 'גילוי מסמכים', date: getDeadline({ startDate: lastDocAsDate,  after: 30}), },
    { name: 'עיון במסמכים', date: getDeadline({ startDate: lastDocAsDate,  after: 60}), },
    { name: 'גילוי ספציפי', date: getDeadline({ startDate: lastDocAsDate,  after: 60}), },
    pretrialAsDate && { name: 'רשימת עדים (תובע)', date: getDeadline({ startDate: pretrialAsDate,  before: 20}), },
    pretrialAsDate && { name: 'רשימת עדים', date: getDeadline({ startDate: pretrialAsDate,  before: 7}), },
  ]
    .filter(Boolean)
    .sort((a, b) => {
